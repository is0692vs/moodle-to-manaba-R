...
function findCourseCards() {
  ...
  if (placeholders.length > 0) {
    console.log(
      "[M2M] Found loading placeholders:",
      placeholders.length,
      "- content may still be loading"
    );

    // Check if there are any real course elements alongside placeholders
    const realCourseElements = document.querySelectorAll(
      'a[href*="course/view.php"], .coursename a, [data-course-id]'
    );
    ...
    if (realCourseElements.length === 0) {
      ...
      console.log(
        "[M2M] Found some real course elements, proceeding with extraction"
      );
    }
  }
  ...
  if (allCourseLinks.length > 0) {
    ...
    console.log(
      "[M2M] Valid course links (not in placeholders):",
      validLinks.length
    );
    ...
    if (validLinks.length > 0) {
      ...
      return validLinks
        .map(
          (link) =>
            link.closest("div, li, article, .card, .course-listitem") ||
            link.parentElement
        )
        .filter(Boolean);
    }
  ...
  for (const selector of ALTERNATIVE_SELECTORS.courseCards) {
    ...
    if (cards.length > 0) {
      console.log(
        "[M2M] Found course cards with selector:",
        selector,
        "count:",
        cards.length
      );
      ...
      for (let i = 0; i < cards.length; i++) {
        ...
        const hasRealContent =
          card.querySelector(
            '.coursename, a[href*="course/view.php"], .course-title'
          ) ||
        ...
        if (!hasPlaceholder && hasRealContent) {
          realCards.push(card);
          console.log(
            `[M2M] Real card ${i + 1} HTML:`,
            card.outerHTML.substring(0, 300)
          );
        }
      }
...
function waitForCourseContainer() {
  ...
  const bodyObserver = new MutationObserver((mutations) => {
        ...
        mutation.addedNodes.forEach((node) => {
          if (node.nodeType === Node.ELEMENT_NODE) {
            ...
            if (
              ...
              shouldCheck = true;
              console.log(
                "[M2M] Course content detected in mutation:",
                element
              );
            }
          }
    ...
    if (shouldCheck) {
      const target = findElement(ALTERNATIVE_SELECTORS.courseViewContent);
      console.log(
        "[M2M] Checking for course container after content change:",
        !!target
      );
      if (target) {
...
function scheduleProcessing() {
  ...
  queueMicrotask(async () => {
    ...
    console.log(
      "[M2M] Looking for course cards... (attempt",
      retryCount + 1,
      "of",
      MAX_RETRIES,
      ")"
    ...
    if (!cards.length) {
      ...
      if (retryCount < MAX_RETRIES) {
        const delay = Math.min(
          BASE_RETRY_DELAY_MS + retryCount * RETRY_DELAY_INCREMENT_MS,
          MAX_RETRY_DELAY_MS
        ); // Progressive delay
        ...
        showStatus(
          "コース情報の読み込みに時間がかかっています。ページを更新してください。"
        );
      }
      return;
    }
...
async function processCourseCards(courseCards) {
  ...
  try {
    ...
    const courses = loadedCourses.filter((course) => {
      ...
      console.log(
        "[M2M] Course loaded with schedule:",
        course.name,
        "Schedule:",
        course.schedule
      );
    ...
    if (wrapper) {
      ...
      if (table) {
        ...
        console.log(
          "[M2M] Table computed style display:",
...
function extractCourseInfos(courseCards) {
  ...
  courseCards.forEach((card) => {
    ...
    for (const selector of linkSelectors) {
      ...
      if (link) {
        console.log(
          "[M2M] Found course link with selector:",
          selector,
          "Link:",
          link
        );
    ...
    if (!link) {
      ...
      while (parent && parent !== document.body) {
        for (const selector of linkSelectors) {
          ...
          if (link) {
            console.log(
              "[M2M] Found course link in parent with selector:",
              selector
            );
            break;
          }
    ...
    if (!link) {
      ...
      if (nameElement) {
        console.log(
          "[M2M] Course name found but no link - this might be a display-only element"
        );
      }
      return;
    }
...
function renderTimetable(courses) {
  try {
    ...
    if (typeof generateManabaTable !== "function") {
      throw new Error(
        "generateManabaTable function not found - tableGenerator.js may not be loaded"
      );
    }
...
function hideOriginalCourseView() {
  console.log(
    "[M2M] Minimizing original course view (not hiding completely)..."
  );
  const target = findElement(ALTERNATIVE_SELECTORS.courseViewContent);
  if (target) {
  ...
  try {
    ...
    styleTag.textContent = `
      ...
      .courselistweekly-nonborder {
        background: ${colors.courseCellColor} !important;
        border-color: ${colors.borderColor} !important;
      }
      
      .stdlist th.top {
        background: ${colors.headerColor} !important;
      }
      
      .courselistweekly-c a,
      .course-link {
...
function monitorNetworkActivity() {
  ...
  const checkLoadingComplete = () => {
    ...
    if (
      ...
      indicators.length === 0 &&
      placeholders.length < MAX_LOADING_PLACEHOLDERS
    ) {
      console.log(
        "[M2M] Network activity appears complete, triggering course processing"
      );
